...
<!-- # # Function to Preprocessing and n-grams creation -->
<!-- # preprocess_and_create_ngrams <- function(df, preprocessing_type, n_gram_size = 2) { # Creation function to preprocess and create n-grams and  -->
<!-- #   df %>% -->
<!-- #     anti_join(stop_words_df, by = "word") %>% # Eliminar stop words -->
<!-- #     {if (preprocessing_type == "stem") mutate(., word = wordStem(word, language = "en")) else .} %>% # Aplicar stemming si es necesario -->
<!-- #     {if (preprocessing_type == "lemma_textstem") mutate(., word = textstem::lemmatize_words(word)) else .} %>% # Aplicar lematización textstem si es necesario -->
<!-- #     unnest_ngrams(ngram, word, n = n_gram_size) %>% # Crear n-gramas -->
<!-- #     {if (n_gram_size > 1) separate(., ngram, paste0("word", 1:n_gram_size), sep = " ") else .} %>% # Separar n-gramas (solo si n > 1) -->
<!-- #     {if (n_gram_size > 1) unite(., ngram, paste0("word", 1:n_gram_size), sep = " ") else .} %>% # Unir n-gramas (solo si n > 1) -->
<!-- #     count(review_id, ngram, sort = TRUE) %>% # Contar la frecuencia de los n-gramas -->
<!-- #     bind_tf_idf(ngram, review_id, n) # Calcular TF-IDF -->
<!-- # } -->
<!-- # ``` -->
