...
word_counts %>%
  ...
  mutate(word = reorder(word, n),
         color = ifelse(row_number() == 1, "red", "blue")) %>% # Color condicional
  ...
  labs(title = "Top 20 Most Common Words (Con Stop Words)", x = "Words", y = "Frequency") +
  theme_minimal() +
...
# Calculating the maximum frequency for the plot after removing stopwords
max_freq_no_sw <- word_counts %>%
...
word_counts %>%
  ...
  scale_color_identity() + # Importante: usar scale_color_identity()
  coord_flip() +
  expand_limits(y = c(0, max_freq_no_sw * 1.1)) + # Usar el nuevo máximo
  labs(title = "Top 20 Most Common Words (Sin Stop Words)", x = "Words", y = "Frequency") +
  theme_minimal() +
  theme(panel.grid.major.y = element_blank(), axis.text.y = element_text(size = 12)) +
  scale_y_continuous(labels = comma)
```
