...
```{r data inspection after removing stopwords, warning=FALSE}
# Remove stop words and create the plot
word_counts %>%
  anti_join(stop_words_df, by = "word") %>%
  top_n(20, n) %>%
  mutate(word = reorder(word, n)) %>% # Important: reorder AFTER filtering
  ggplot(aes(x = word, y = n)) +
  geom_segment(aes(xend = word, yend = 0), size = 1.2, color = "skyblue") + # Lollipop stems
  geom_point(size = 4, color = "blue") + # Lollipop points
  coord_flip() +
  labs(title = "Top 20 palabras más frecuentes (sin Stop Words)", x = "Palabras", y = "Frecuencia") +
  theme_minimal() +
  theme(panel.grid.major.y = element_blank(),
        axis.text.y = element_text(size = 12)) # Improve y-axis text size
```
