...
word_counts %>%
  ...
  mutate(word = reorder(word, n), # Esto va dentro de mutate
         color = case_when(       # Esto también va dentro de mutate
           word == top_word_no_sw ~ "forestgreen",
  ...
  labs(title = "Top 20 Most Common Words (Con Stop Words, 2 destacados)", x = "Words", y = "Frequency") +
  theme_minimal() +
  theme(panel.grid.major.y = element_blank(), axis.text.y = element_text(size = 12)) +
...
word_counts %>%
  ...
  mutate(word = reorder(word, n),
         color = ifelse(row_number() == 1, "forestgreen", "blue")) %>% # Color condicional
  ggplot(aes(x = word, y = n, color = color)) +
  ...
  coord_flip() +
  expand_limits(y = c(0, top_word_with_sw * 1.1)) + # Usar el nuevo máximo
...
once we have eliminated the stopwords, we can see the most common words in the reviews are, as we could expect, words like "movie" or "film" which is something obvious, because of the origin of the dataset.
