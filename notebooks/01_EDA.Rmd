...
word_counts_lemma %>%
  ...
  labs(title = "Top 20 Most Common Words (Lemmatized, No Stopwords)", x = "Words", y = "Frequency") +
  theme_minimal() +
  theme(text = element_text(size = 10, family = "Montserrat"),
        panel.grid.major.y = element_blank(), 
        axis.text.y = element_text(size = 12, family = "Montserrat"), 
...
lemma_df <- udpipe(tokenized_reviews$text, udmodel) %>% #lematizacion
    as_tibble() %>% #convertir a tibble
    filter(upos %in% c("NOUN", "ADJ", "VERB", "ADV")) %>% #filtrado de palabras
    select(lemma,sentence) %>% #seleccion de columnas
    group_by(lemma) %>% #agrupacion por lema
    count() %>% #conteo
    arrange(desc(n)) #ordenamiento

print(as_tibble(lemma_df), n = 5)
```

and the distribution of the model 

```{r }
# Calcular el máximo DESPUÉS de la lematización
max_top_word_with_sw_lemma_udpipe <- lemma_df %>%
  pull(n) %>%
  max()
...
lemma_df %>%
  ...
  mutate(lemma = reorder(lemma, n),
         color = ifelse(row_number() == 1, "orange", "blue")) %>%
  ggplot(aes(x = lemma, y = n, color = color)) +
...
This last execution is more complex due to the use of the model of language which is more accureta than the function `lemmatize_words()` but consummes a lot of resources. Nevertheless it has support in other languages, besides english.
